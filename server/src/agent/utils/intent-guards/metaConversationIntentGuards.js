...
} from "../conversation/uiNavigationFeedbackGuards.js";

export { isNexxusCockpitUiDiscussion } from "../conversation/uiNavigationFeedbackGuards.js";
...
const CAPABILITY_OVERVIEW_PATTERNS = [
  /\b(quelles? sont tes|quels sont tes|quelle est ton|quel est ton|tes)\s+(fonctionnalit|capacit|possibilit|competenc|aptitud|savoir[- ]?faire|expertise)/,
  /\b(fonctionnalit|capacit|competenc|aptitud|savoir[- ]?faire|expertise).{0,24}phares?\b/,
  /\b(qu est ce que tu peux|ce que tu peux)\s+(faire|m expliquer|m dire)\b/,
  /\b(qu est[- ]?ce que tu sais|ce que tu sais)\s+faire\b/,
  /\bque sais[- ]?tu faire\b/,
  /\b(que peux tu faire|tu peux faire quoi)\b/,
  /\ba quoi sers[- ]?tu\b/,
  /\b(presente tes|decris tes)\s+(fonctionnalit|capacit|competenc|aptitud|savoir[- ]?faire|expertise|services)\b/,
...
const HELP_SCOPE_PATTERNS = [
  ...
  /\bdans quels domaines (peux[- ]?tu|tu peux) m aider\b/,
...
export function classifyMetaConversationIntent(query = "") {
  ...
  if (matchesAny(text, CAPABILITY_OVERVIEW_PATTERNS)) {
    // Motif déjà complet (« what can you do ») : ne pas renvoyer en reflective
    // à cause d'un dernier token court (do / tu).
    const tier = wc > 22 ? "reflective" : "deterministic";
    return {
