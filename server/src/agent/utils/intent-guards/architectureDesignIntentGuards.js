...
export const ARCHITECTURE_DESIGN_SPREADSHEET_EXCLUSION_RULE =
  "spreadsheet_build_excludes_architecture_design";
...
/** Tableur / dashboard Excel — conception fonctionnelle, pas 3 approches RAG. */
const SPREADSHEET_ARTIFACT_RE =
  /\b(?:excel|xlsx|spreadsheet|classeurs?|google\s*sheets?|microsoft\s+365|tableau(?:x)?\s+de\s+bord|dashboards?)\b/i;
...
/** OS / desktop / GUI — brief système, pas seulement agent/pipeline. */
const DESKTOP_SYSTEM_SURFACE_RE =
  /\b(?:systeme d['']?exploitation|operating system|\bos\b|interface graphique|environnement de bureau|desktop(?: environment)?|window manager|barre (?:des )?taches|menu demarrer|fenetres?)\b/i;

const APP_SYSTEM_SURFACE_RE =
  /\b(?:navigateur|browser|calculatrice)\b/i;

function hasDesktopOrAppSystemSurface(q = "") {
  return DESKTOP_SYSTEM_SURFACE_RE.test(q) || APP_SYSTEM_SURFACE_RE.test(q);
}

function hasArchitectureCreateDesire(q = "") {
  if (ARCHITECTURE_DESIRE_CREATE_PATTERNS.some((pattern) => pattern.test(q))) {
    return true;
  }
  if (
    /\b(?:peux|pourrais|pourras)[- ]?tu\s+m\s+aider.{0,48}\b(?:creer|construire|developper|concevoir)\b/.test(
      q,
    )
  ) {
    return true;
  }
  return /\bconcevoir\s+(?:un|une|l['']|la |le )\b/.test(q);
}
...
/** Demande explicite de profondeur — pas le brief create lui-même. */
const ARCHITECTURE_IMMEDIATE_DEPTH_RE =
  /\b(?:compare(?:r|s|z)?|comparaison|d[eé]taille(?:r|s|z)?|approfondis(?:s)?(?:ez)?|en profondeur|plan complet|plan d[eé]taill[eé]|d[eé]veloppe(?:r|s|z)? le plan|montre(?:r|s|z)? le code|donne(?:r|s|z)? le code|g[eé]n[eè]re(?:r|s|z)? le code|recherche(?:r|s|z)?(?: web| en ligne)?|cherche(?:r|s|z)? sur)\b/i;

const ARCHITECTURE_HOW_TO_PATTERNS = [
...
const ARCHITECTURE_DESIRE_CREATE_PATTERNS = [
  J_AIMERAIS_CREATE_RE,
  /\bje (?:voudrais|veux|souhaite) (?:creer|créer|construire|mettre en place|developper|développer|concevoir)\b/,
];
...
/**
 * Excel / tableur / tableau de bord + intention de création → pas architecture_design.
 * @param {string} query
 */
export function isSpreadsheetBuildExclusionForArchitectureDesign(query = "") {
  const q = normalizeArchitectureDesignQuery(query);
  if (!q) return false;
  if (!WEB_ARTIFACT_CREATE_RE.test(q)) return false;
  if (!SPREADSHEET_ARTIFACT_RE.test(q)) return false;
  if (EXPLICIT_SYSTEM_ARCHITECTURE_RE.test(q)) return false;
  return true;
}
...
/**
 * Création tableur / Excel / dashboard — livrable, pas analyse vidéo ni cadrage.
 * @param {string} query
 */
export function isSpreadsheetCreateRequest(query = "") {
  const q = normalizeArchitectureDesignQuery(query);
  if (!q || q.length < 12) return false;
  if (!SPREADSHEET_ARTIFACT_RE.test(q)) return false;
  if (EXPLICIT_SYSTEM_ARCHITECTURE_RE.test(q)) return false;
  return SPREADSHEET_CREATE_VERB_RE.test(q) || WEB_ARTIFACT_CREATE_RE.test(q);
}
...
function isGuidedCreationInsteadOfArchitecture(query = "") {
  ...
  if (isSpreadsheetBuildExclusionForArchitectureDesign(query)) return true;
  if (
...
export function isArchitectureDesignIntent(query = "") {
  ...
  if (isSpreadsheetBuildExclusionForArchitectureDesign(query)) return false;
  if (isExploitableProcedureIntent(query)) return false;
  if (getArchitectureDesignWordCount(query) > ARCHITECTURE_DESIGN_MAX_WORDS) {
    // ponytail: brief OS/GUI souvent > 120 mots ; plafond dur 200.
    if (!hasDesktopOrAppSystemSurface(q) || getArchitectureDesignWordCount(query) > 200) {
      return false;
    }
  }
  ...
  if (ARCHITECTURE_HOW_TO_PATTERNS.some((pattern) => pattern.test(q))) return true;
  if (
    hasArchitectureCreateDesire(q) &&
    (EXPLICIT_SYSTEM_ARCHITECTURE_RE.test(q) || hasDesktopOrAppSystemSurface(q))
  ) {
    return true;
  }
  return false;
...
export function isArchitectureHowToRequest(query = "") {
  return ARCHITECTURE_HOW_TO_PATTERNS.some((pattern) =>
    pattern.test(normalizeArchitectureDesignQuery(query)),
  );
}

export function isArchitectureImmediateDepthRequest(query = "") {
  return ARCHITECTURE_IMMEDIATE_DEPTH_RE.test(
    normalizeArchitectureDesignQuery(query),
  );
}

/**
 * Brief create sans HOW_TO / profondeur explicite → quick + deferred.
 * @returns {{ analysisMode: "deferred"|"immediate", depthTrigger: "default"|"how_to"|"explicit_detail", responseMode: "quick"|"open_proposition" }}
 */
export function resolveArchitectureDepthControl(query = "") {
  if (isArchitectureImmediateDepthRequest(query)) {
    return {
      analysisMode: "immediate",
      depthTrigger: "explicit_detail",
      responseMode: "open_proposition",
    };
  }
  if (isArchitectureHowToRequest(query)) {
    return {
      analysisMode: "immediate",
      depthTrigger: "how_to",
      responseMode: "open_proposition",
  ...
  if (isCreateMandateRequest(query) || hasArchitectureCreateDesire(normalizeArchitectureDesignQuery(query))) {
    return {
      analysisMode: "deferred",
      depthTrigger: "default",
      responseMode: "quick",
    };
  }
  return {
...
export function shouldFinalizeArchitectureDesignRail(shortCircuit = null) {
  return (
    shortCircuit?.path === "architecture_design_deterministic" &&
    Boolean(String(shortCircuit.reply || "").trim())
  );
}

export function applyArchitectureDepthMetrics(turnTelemetry, shortCircuit = {}, startedAt = 0) {
  const routingMs = Math.max(0, Math.round(Date.now() - Number(startedAt || 0)));
  const analysisMode = shortCircuit.analysisMode || "immediate";
  const depthTrigger = shortCircuit.depthTrigger || "default";
  turnTelemetry?.setMetric?.("routing_ms", routingMs);
  turnTelemetry?.setMetric?.("ttft_ms", routingMs);
  turnTelemetry?.setMetric?.("generation_ms", 0);
  turnTelemetry?.setMetric?.("analysis_mode", analysisMode);
  turnTelemetry?.setMetric?.("depth_trigger", depthTrigger);
  turnTelemetry?.setMetric?.("ttft", routingMs);
}
...
export function extractArchitectureTopic(query = "") {
  ...
  for (const pattern of patterns) {
    ...
    if (!match?.[1]) continue;
    let topic = String(match[1] || "").trim();
    if (!hasDesktopOrAppSystemSurface(q)) {
      topic = topic
        .replace(/\s+qui\b.*$/, "")
...
const CODE_REVIEW_TEMPLATE_LICENSE_RE =
  /\b(?:code[- ]?reviewer|revue de code|reviewer|linter|index(?:ation)?(?:\s+(?:du|de|un))?\s+code|fichiers? source|server\/src|d[eé]p[oô]t de code)\b/;

/** Gabarit revue-de-code / server/src seulement si le tour courant le license. */
export function isCodeReviewArchitectureTemplateLicensed(query = "") {
  return CODE_REVIEW_TEMPLATE_LICENSE_RE.test(
    normalizeArchitectureDesignQuery(query),
  );
}
...
export function buildArchitectureDesignOptionsReply(query = "") {
  ...
  if (isCodeReviewArchitectureTemplateLicensed(query)) {
...
Tu vises plutôt une architecture conceptuelle, un prototype rapide, ou une implémentation complète ?`;
  ...
  const ragHint = /\brag\b/.test(q);
  const midLabel = ragHint
    ? "Approche intermédiaire (RAG + règles)"
    : "Approche intermédiaire";
  const midBody = ragHint
    ? "Index partiel + règles métier. Premier pas : indexer un corpus représentatif (3 documents), pas un dossier code par réflexe."
    : "Cadrage fonctionnel + un premier périmètre mesurable. Premier pas : figer acteurs, entrée/sortie et critère de succès v1.";

  return `Pour ${topic}, voici 3 approches distinctes :
1. **Approche légère** — Prototype minimal pour valider l'idée. Premier pas : un flux unique, sans usine.
2. **${midLabel}** — ${midBody}
3. **Approche industrielle** — Pipeline et ops à grande échelle — seulement si le besoin d'échelle est déjà prouvé. Premier pas : cadrer métriques et budget.
...
export function buildArchitectureDesignFramingReply(query = "") {
  ...
  if (topic.length > 90) {
    const cut = topic.slice(0, 90);
    const lastSpace = cut.lastIndexOf(" ");
    shortTopic = `${(lastSpace > 40 ? cut.slice(0, lastSpace) : cut).trim()}…`;
  }
