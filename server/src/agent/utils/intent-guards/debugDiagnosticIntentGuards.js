...
import { hasPostRepairSocialClose } from "../../policies/social/postRepairSocialClosePolicy.js";
...
export function isAssistantResponseQualityFeedback(query = "") {
  ...
  if (!q) return false;
  return (
    /\b(?:ta|votre|cette|la)\s+r[eé]ponse\b.{0,50}\b(?:[eé]chec|incorrecte?|invalide|mauvaise|bizarre|chelou|[eé]trange|pas\s+(?:correcte?|bonne|ok|ça|ca)|ratee?|ratée)\b/i.test(
      q,
    ) ||
    /\b(?:c['']est|ce n['']est)\s+(?:pas\s+)?(?:une?\s+)?(?:bonne\s+)?r[eé]ponse\b/i.test(
      q,
    ) ||
    /\br[eé]ponse\s+(?:est|était)\s+(?:un\s+)?(?:[eé]chec|incorrecte?|hors\s+sujet|bizarre)\b/i.test(
      q,
    ) ||
    /\b(?:tu as|t['']as)\s+(?:mal|pas)\s+(?:r[eé]pondu|compris)\b/i.test(q) ||
    // « induit en erreur » = méprise conversationnelle, pas incident technique.
...
export function isDebugDiagnosticSignal(query = "") {
  ...
  // Incident conversationnel déjà refermé par l'utilisateur — pas un ticket tech.
  if (hasPostRepairSocialClose(query)) return false;
  return (
