...
import { isCasualExplanationFollowUp, isWellbeingCheckinIntent } from "../../policies/social/index.js";
...
export function classifyIntent(query = "", context = {}) {
  ...
  if (isWellbeingCheckinIntent(query)) {
    return {
      intent: INTENT_TAXONOMY.SOCIAL_CHIT_CHAT,
      budget: REASONING_BUDGET.SOCIAL_CHIT_CHAT,
      bypassDirectAnswer: false,
      reason: "wellbeing_checkin_social_priority",
      scores: { socialTotal: 6, expertScore: 0, technicalScore: 0, actionScore: 0, complexityScore: 0, contextDelta: 0 },
    };
  }
  ...
  const socialWords = [
    ...
    'discuter', 'parler', 'ca va', 'comment vas tu', 'comment allez vous',
    'comment allez-vous', 'comment vous allez', 'merci', 'super', 'genial'
  ];
