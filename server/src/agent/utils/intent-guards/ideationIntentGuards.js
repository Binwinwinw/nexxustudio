...
export function isIdeationIntent(query = "") {
  ...
  // « je veux créer une carte de visite » = livrable nommé, pas idéation ouverte.
  if (
    /\bje veux (?:creer|construire)\s+(?:un|une)\s+(?!projet\b|truc\b|chose\b|idee\b)/.test(
      q,
    )
  ) {
    return false;
...
const PROJECT_CRITIQUE_RE =
  /\b(?:parait(?:-il)? pertinent|est[- ]ce (?:que )?(?:ce projet |ca |cela )?pertinent|quen penses[- ]tu|trouve(?:s|r)?(?: moi)? des failles|ne me complimente pas|ne soi[st] pas (?:obligatoirement )?d accord|pose des questions? si necessaire)\b/;

const PROJECT_IDEA_HOOK_RE =
  /\b(?:je (?:voudrais|veux|souhaite)|j aimerais|jaimerais)\s+(?:creer|construire|faire|lancer)|\b(?:generateur|blagues?|projet|idee)\b/;

/**
 * Idée de projet à évaluer (pertinence / failles) — pas build, pas matrice RAG.
 * @param {string} query
 */
export function isProjectIdeaCritiqueRequest(query = "") {
  const q = normalizeIdeationQuery(query);
  if (!q || q.length < 24) return false;
  if (!PROJECT_CRITIQUE_RE.test(q)) return false;
  return PROJECT_IDEA_HOOK_RE.test(q);
}

export function buildProjectIdeaCritiqueSystemAddon(query = "") {
  const q = String(query || "").trim();
  return [
    "VARIANTE CRITIQUE D'IDÉE (pas architecture, pas exécution, pas compliments) :",
    `Demande : « ${q.slice(0, 280)} »`,
    "",
    "RÈGLES :",
    "1) Juge la pertinence de CETTE idée. Interdit d'être d'accord par réflexe.",
    "2) Cherche des failles concrètes (saturation marché, différenciation, data, humour généré plat, coût/valeur, éthique).",
    "3) 1–3 questions seulement si un fait manque pour juger.",
    "4) INTERDIT : matrice « 3 approches », RAG + règles, code-reviewer, indexation server/src, « Je partirais plutôt sur ».",
    "5) INTERDIT : sujet collant d'un tour précédent. Reste sur l'idée nommée ici.",
    "",
    "FORMAT : 2–4 paragraphes courts. Tutoiement. Pas de flatterie.",
  ].join("\n");
}

export function resolveProjectIdeaCritiqueShortCircuit(query = "") {
  if (!isProjectIdeaCritiqueRequest(query)) return null;
  return {
    path: "project_idea_critique",
    deferToLlm: true,
    projectIdeaCritique: true,
    reflectiveHint: buildProjectIdeaCritiqueSystemAddon(query),
    step: "🔎 Critique d'idée — failles d'abord (sans architecture)...",
  };
}
