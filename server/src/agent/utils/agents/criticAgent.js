import { TOOL_REGISTRY } from '../runtime/toolRegistry.js';
import turnTelemetry from '../../telemetry/turnTelemetry.js';
import syntaxProxy from '../parsing-normalization/syntaxProxy.js';
import { getClientForModel } from '../../../llm/llmFactory.js';
import { AGENT_ROLES } from '../../policies/core/index.js';
import { reconcileCriticFileUseVerdict } from '../../policies/attachment/attachmentReadMandatePolicy.js';
...
class CriticAgent {
   ...
   */
  async evaluateReflexionContract({ user_query, execution_contract, forbidden_flags, tools_used, raw_answer, document_briefing = "" }) {
    ...
    const excerpt = String(document_briefing || "").replace(/\s+/g, " ").trim().slice(0, 2400);
...
- document_briefing_excerpt : ${excerpt || "(aucun briefing fichier injecté)"}

[RÉPONSE À ÉVALUER]
${raw_answer}

[INSTRUCTION]
Pour file_not_used / generic_answer_without_document, compare raw_answer au document_briefing_excerpt.
Si la réponse reprend des faits, titres ou extraits visibles du briefing : ne pose pas ces flags.
Génère le JSON strict correspondant à ton verdict. Ne renvoie AUCUN autre texte.`;
    ...
    try {
      ...
      return reconcileCriticFileUseVerdict(parsed, raw_answer, document_briefing);
    } catch (err) {
  ...
  async applyLabel(response, report) {
    const groundTruthService = (await import('../context/groundTruthService.js')).default;
