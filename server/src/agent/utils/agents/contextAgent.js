...
class ContextAgent {
  ...
  async _readFileContent(file) {
    ...
    try {
      ...
      if (/\.html?$/i.test(fileName) || /text\/html/i.test(String(file.mimetype || ""))) {
        const htmlViews = extractHtmlDocumentViews(raw, {
          fileName,
          mime: file.mimetype || "text/html",
          bytes: file.size || Buffer.byteLength(raw, "utf8"),
        });
        const structured = formatHtmlDocumentBriefing(htmlViews);
        return {
          text: structured ? `${structured}\n\n[HTML BRUT — non prioritaire]\n${raw.slice(0, 2000)}` : raw,
          htmlViews,
        };
      }
      return { text: raw };
  ...
  async ingest(files) {
    ...
    for (const file of textFiles) {
      const { text, pdfMeta, htmlViews } = await this._readFileContent(file);
      contents.push({
