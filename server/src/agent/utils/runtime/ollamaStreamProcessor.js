...
import responseThinkingCleaner from '../quality-safety/responseThinkingCleaner.js';
import { recoverVisibleFromFullResponse } from '../conversation/genericGreetingGuards.js';

const THINK_TAG_OPEN = '<' + 'redacted_thinking' + '>';
const THINK_TAG_OPEN_LEGACY = '<' + 'think' + '>';
...
const THINK_OPEN_TAGS = [THINK_TAG_OPEN, THINK_TAG_OPEN_LEGACY];
const THINK_CLOSE_TAGS = [THINK_TAG_CLOSE, THINK_TAG_CLOSE_LEGACY];

function findFirstTag(buffer, tags) {
  const candidates = tags
    .map((tag) => ({ index: buffer.indexOf(tag), length: tag.length }))
    .filter((c) => c.index !== -1);
...
function findThinkOpen(buffer) {
  return findFirstTag(buffer, THINK_OPEN_TAGS);
}

function findThinkClose(buffer) {
  return findFirstTag(buffer, THINK_CLOSE_TAGS);
}

function longestHeldTagPrefix(buffer, tags, { ignoreCase = false } = {}) {
  const hay = ignoreCase ? buffer.toUpperCase() : buffer;
  let keepLen = 0;
  for (const tag of tags) {
    const needle = ignoreCase ? tag.toUpperCase() : tag;
    for (let i = 1; i < needle.length; i++) {
      if (hay.endsWith(needle.slice(0, i))) {
...
class OllamaStreamProcessor {
  ...
  processToken(token) {
    ...
    while (this.buffer.length > 0) {
      if (this.state === 'TEXT') {
        const { index: thoughtIndex, length: thoughtOpenLen } = findThinkOpen(
          this.buffer,
        );
        const { index: orphanCloseIndex, length: orphanCloseLen } =
          findThinkClose(this.buffer);
        ...
        let nextTagIndex = -1;
        const candidates = [
          { idx: thoughtIndex, len: thoughtOpenLen, state: 'THINK' },
          { idx: orphanCloseIndex, len: orphanCloseLen, state: 'THINK_CLOSE_ORPHAN' },
        ...
        if (nextTagIndex === -1) {
          const keepLen = Math.max(
            longestHeldTagPrefix(this.buffer, [
              ...THINK_OPEN_TAGS,
              ...THINK_CLOSE_TAGS,
              ACTION_TAG_OPEN,
            ]),
            longestHeldTagPrefix(this.buffer, [READY_TAG], { ignoreCase: true }),
          );
          ...
          if (matched.state === 'THINK') {
            this.state = 'THINK';
            this.currentThought = '';
          } else if (matched.state === 'THINK_CLOSE_ORPHAN') {
            // `</think>` isolé (Qwen) : jamais visible, rester en TEXT.
        ...
        if (endThoughtIndex === -1) {
          const keepLen = longestHeldTagPrefix(this.buffer, THINK_CLOSE_TAGS);
  ...
  finalize() {
    if (this.buffer) {
      if (this.state === 'TEXT') {
        const dangling = longestHeldTagPrefix(this.buffer, [
          ...THINK_OPEN_TAGS,
          ...THINK_CLOSE_TAGS,
          ACTION_TAG_OPEN,
        ]);
        const visibleTail = dangling === this.buffer.length ? '' : this.buffer;
        this.currentResponse += visibleTail;
