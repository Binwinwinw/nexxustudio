...
# 1. Update criticAgent.js
with open(os.path.join(ROOT, 'server/src/agent/utils/criticAgent.js'), 'r', encoding='utf-8') as f:
    critic_content = f.read()

deterministic_methods = r"""
  /**
...
if "evaluateInlineFileAnalysis" not in critic_content:
    critic_content = critic_content.replace("  async applyLabel(response, report) {", deterministic_methods + "\n  async applyLabel(response, report) {")
    with open(os.path.join(ROOT, 'server/src/agent/utils/criticAgent.js'), 'w', encoding='utf-8') as f:
        f.write(critic_content)
    print("criticAgent.js updated successfully")

# 2. Update SovereignOrchestrator.js
with open(os.path.join(ROOT, 'server/src/agent/orchestrator/SovereignOrchestrator.js'), 'r', encoding='utf-8') as f:
    orch_content = f.read()

new_intent_logic = r"""  _evaluateFileDrivenIntent(query, attachments) {
...
# Regex substitution to replace _evaluateFileDrivenIntent
orch_content = re.sub(r'  _evaluateFileDrivenIntent[\s\S]*?return null;\n  }', new_intent_logic, orch_content, count=1)

# Loop critique replacement
old_loop_critique = """        const tools_used = packet.expert_outputs.map(o => o.stage);
...
new_loop_critique = """        const tools_used = packet.expert_outputs.map(o => o.stage);
        let critique;
...
if "evaluateInlineFileAnalysis" not in orch_content:
    orch_content = orch_content.replace(old_loop_critique, new_loop_critique)
    with open(os.path.join(ROOT, 'server/src/agent/orchestrator/SovereignOrchestrator.js'), 'w', encoding='utf-8') as f:
        f.write(orch_content)
    print("SovereignOrchestrator.js updated successfully")
